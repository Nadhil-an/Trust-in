      ...
      {showModal && (
        <Modal isOpen={true} onClose={()=>setShowModal(false)} title={selected?"Edit User":"Add User"}
          ...
          <form id="usr-form" onSubmit={handleSave}>
            ...
            <div className="form-group"><label className="form-label required">Role</label><select className="form-control" value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))}>{["MANAGER","ACCOUNTANT","CASHIER","HR","ADMIN"].map(r=><option key={r}>{r}</option>)}</select></div>
