      ...
      {showModal && (
        <Modal isOpen={true} onClose={()=>{setShowModal(false); setModalError(null);}} title={selected?"Edit User":"Add User"}
          ...
          <form id="usr-form" onSubmit={handleSave}>
            ...
            <div className="form-group"><label className="form-label required">Role</label><select className="form-control" value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))}>{[...webRoles, ...mobileRoles].map(r=><option key={r} value={r}>{r.replace(/_/g, ' ')}</option>)}</select></div>
