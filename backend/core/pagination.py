...
class StandardResultsSetPagination(PageNumberPagination):
    ...
    def get_paginated_response(self, data):
        ...
        try:
            ...
            try:
                # Use account_type field (CASH / BANK) — most reliable source
                cash_total = queryset.filter(account_type='CASH').aggregate(total=Sum('amount'))['total'] or 0
                online_total = queryset.filter(account_type='BANK').aggregate(total=Sum('amount'))['total'] or 0
                response.data['total_cash'] = float(cash_total)
                response.data['total_online'] = float(online_total)
            except Exception:
                # Fallback: use payment_method if account_type doesn't exist on this model
                try:
